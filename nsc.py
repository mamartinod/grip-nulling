...
def nsc_function(bins0, na, mu_opd, sig_opd, *args):
    ...
    for _ in range(nloop):
        ...
        rv_sigma_eps = gff.rv_generator(sigma_eps_axis, sigma_eps_cdf, n_samp_per_loop)
...
def run_nsc(activates, skip_fit, chi2_map_switch, maps_sz, nbs, which_nulls,
    ...
    #
                ...
                sigma_eps_bins = (sigma_eps_histo[1][:-1] + sigma_eps_histo[1][1:]) / 2
                rv_sigma_histo = np.histogram(cp.asnumpy(rv_sigma_eps), int(cp.asnumpy(rv_sigma_eps).size**0.5), density=True)
                rv_sigma_bins = (rv_sigma_histo[1][:-1] + rv_sigma_histo[1][1:]) / 2
                f = plt.figure(figsize=(19.20, 10.80))
                plt.plot(sigma_eps_bins, sigma_eps_histo[0], 'o', markersize=5, label='sigma eps')
                plt.plot(rv_sigma_bins, rv_sigma_histo[0], '+', markersize=8, lw=3, label='rv')
                plt.grid()
                plt.legend(loc='best', fontsize=15)
                plt.xlabel('Flux (AU)', size=20)
                plt.ylabel('Frequency', size=20)
